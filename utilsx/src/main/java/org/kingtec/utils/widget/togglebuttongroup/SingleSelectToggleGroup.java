...
import org.kingtec.utils.ui.CustomButton;

import org.kingtec.utils.widget.togglebuttongroup.button.MarkerButton;
...
public class SingleSelectToggleGroup extends ToggleButtonGroup {
    ...
    public <T> SingleSelectToggleGroup setChooser(List<T> list, Chooser<T> chooser
    ) {

      removeAllViews();
        for (T o : list) {
            CustomButton toggle = new CustomButton(getContext());
            toggle.setText(chooser.title(o));
            toggle.setTag(o);
       addView(toggle);
        }
       setOnCheckedChangeListener((group, checkedId) -> {
            String v = ((CustomButton) group.findViewById(checkedId)).getText().toString();
    ...
    public <T extends Object> T getCheckedTag() {
        return (T)getChildAt(mCheckedId).getTag();
    }
